...
import study.chapter4.Bipartite;
import study.chapter4.Graph;
import study.chapter4.GraphGenerator;
 ...
 * <p>
 * This implementation uses the <em>Hopcroft-Karp algorithm</em>. The order of
 * growth of the running time in the worst case is (<em>E</em> + <em>V</em>)
 * sqrt(<em>V</em>), where <em>E</em> is the number of edges and <em>V</em> is
 * the number of vertices in the graph. It uses extra space (not including the
 * graph) proportional to <em>V</em>.
 * <p>
 * See also {@link BipartiteMatching}, which solves the problem in O(<em>E
 * V</em>) time using the <em>alternating path algorithm</em> and <a href =
 * "https://algs4.cs.princeton.edu/65reductions/BipartiteMatchingToMaxflow.java.html">BipartiteMatchingToMaxflow</a>,
 * which solves the problem in O(<em>E V</em>) time via a reduction to the
 * maxflow problem.
 * <p>
...
public class HopcroftKarp {
	...
	public HopcroftKarp(Graph G) {
		...
		// the call to hasAugmentingPath() provides enough info to reconstruct level
		// graph
	...
	// is the edge v-w a forward edge not in the matching or a reverse edge in the
	// matching?
	...
	private boolean hasAugmentingPath(Graph G) {
		...
		// breadth-first search (starting from all unmatched vertices on one side of
		// bipartition)
	...
	/**
	 * Returns true if the graph contains a perfect matching. That is, the number of
	 * edges in a maximum matching is equal to one half of the number of vertices in
	 * the graph (so that every vertex is matched).
	 *
	 * @return {@code true} if the graph contains a perfect matching; {@code false}
	 *         otherwise
	 */
	...
	/**
	 * Returns true if the specified vertex is in the minimum vertex cover computed
	 * by the algorithm.
	 *
	 * @param v
	 *            the vertex
	...
	// check that mate[] and inVertexCover[] define a max matching and min vertex
	// cover, respectively
	...
	/**
	 * Unit tests the {@code HopcroftKarp} data type. Takes three command-line
	 * arguments {@code V1}, {@code V2}, and {@code E}; creates a random bipartite
	 * graph with {@code V1} + {@code V2} vertices and {@code E} edges; computes a
